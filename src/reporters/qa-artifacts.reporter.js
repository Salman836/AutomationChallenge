...
export default class QaArtifactsReporter {
  ...
  #writeJobSummary(result) {
    ...
    const lines = [`## ${icon} ${projects} — ${result.status}`, '', '| Passed | Failed | Flaky | Skipped | Duration |', '| ---: | ---: | ---: | ---: | ---: |', `| ${passed} | ${failed} | ${flaky} | ${skipped} | ${seconds}s |`, ''];
    ...
    if (flaky > 0) {
      lines.push(`> ⚠️ ${flaky} test(s) passed only on retry. Both suites hit third-party systems, ` + 'so a flake here is usually the network rather than a defect — but a test that ' + 'flakes repeatedly is worth pinning down.', '');
    }
  ...
  #writePerf(result) {
    ...
    this.#write(`${this.perfDir}/perf-summary.json`, JSON.stringify({ startedAt: this.startedAt.toISOString(), status: result.status, entries: this.perf }, null, 2));

    const md = ['# Performance summary', '', `Run started ${this.startedAt.toISOString()} · overall status **${result.status}**`, '', 'All figures are client-observed latency in milliseconds. The first sample of each', 'series is excluded as cold start and reported in its own column. Verdicts are taken', 'on p95 against the budgets in `src/data/perfBudgets.js`.', '', rows.length > 0 ? markdownTable(rows) : '_No latency series recorded._', '', failures.length > 0 ? `## Budget breaches (${failures.length})\n\n` + failures.map((row) => `- **${row.label}** — ${row.reasons.join('; ')}`).join('\n') : '## Budget breaches\n\nNone.', ''].join('\n');
  ...
  #writeObserved() {
    ...
    const md = ['# Observed behaviour', '', '> Generated by `src/reporters/qa-artifacts.reporter.js`. Do not edit by hand.', '', 'Trello does not publish an exact rejection code for every invalid input. These', 'specs assert the class of the response (a 4xx client error) and record what the', 'API actually returned. Use this table to tighten each assertion to an exact', 'status, and to raise a documentation defect wherever the two disagree.', '', '| Case | Endpoint | Expected | Observed | Response body (truncated) |', '| --- | --- | --- | ---: | --- |', ...records.map((r) => `| ${r.caseId} | \`${r.endpoint}\` | ${r.expectation} | **${r.observedStatus}** | ` + `\`${String(r.observedBody).replace(/\|/g, '\\|').slice(0, 160)}\` |`), '', `_${records.length} input(s) recorded at ${new Date().toISOString()}._`, ''].join('\n');
